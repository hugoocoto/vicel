...
This is a community project managed by me, Hugo Coto. I only accept code from
those who contribute out of passion for the project and for programming itself.
I will not accept any code generated by LLMs, as this codebase is, and will
remain, a piece of art, not a corporate-style over-engineered mess of unmanaged
code.

Every modification must be committed only with the changes and files that solve
the same problem. The philosophy behind vicel is to think first, then write
code. All new code must be written in gnu-c11, following c99-style, and
formatted using the .clang-format file located at the root of the project. Any
changes should also be reflected in the documentation, if necessary.

Please discuss new ideas with me before submitting them; I don’t want to reject
anyone’s effort. Thank you very much for contributing to vicel.
